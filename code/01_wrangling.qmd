...
The following code chunk is to conduct data wrangling on the training_trait.csv
...
trait_w <- trait %>%
  ...
  #Feature engineering corn growth stages based on each growth‐state window using planting and harvest date  
  mutate(
...
```
The following code chunk is to conduct data wrangling on  "training_soil.csv"
...
The following code chunk is to merge the 3 training files based on site and year.

```{r}

train_full <- trait_w %>%
  left_join(meta, by = c("site", "year")) %>%
  left_join(soil_w, by = c("site", "year"))
