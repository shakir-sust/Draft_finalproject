...
ggplot() +
  ...
  geom_point(data = tw,
             aes(x = longitude, #"Longitude" goes on longitude
                 y = latitude) #"Latitude" goes on latitude
             )
...
The following code chunk downloads the weather data from "Daymet" for all site - years in the "tw" data frame.

```{r}

?download_daymet()

tw_clean <- tw %>% anti_join(outside_daymet, by = c("hybrid","year","site"))
...
daymet_all <- tw_clean %>% 
  mutate(weather = pmap(list(.y = year, 
                             ...
                             .latitude = latitude, 
                             .longitude = longitude), 
                        function(.y, .site, .latitude, .longitude) 
                          download_daymet( 
                            site = .site, #specifying ".site" placeholder for "site = " argument
                            lat = .latitude, #specifying ".lat" placeholder for "lat = " argument
                            lon = .longitude, #specifying ".lon" placeholder for "lon = " argument
                            start = .y, 
                            end = .y, 
                            simplify = T,
